 ...
 * @Repository
 * 
 * @Transactional public class DoctorRepository_bkup {
 * 
 * 
 * 
 ...
 * template.save(p); }
 * 
 * public Doctor getUser(Integer id) { Doctor d = template.get(Doctor.class,
 * id); return d; }
 * 
 ...
 * c.setProjection(Projections.property("id")); List<Doctor> list =
 * (List<Doctor>) template.findByCriteria(c); Doctor p = null; if
 * (!list.isEmpty()) { p = list.get(0); }
 * 
 * if (p != null) { return Optional.of(p); } return Optional.empty(); }
 * 
 * @SuppressWarnings({ "deprecation", "unchecked" }) public List<Doctor>
 * getAllDoctors(String name, String city, String speciality) { DetachedCriteria
 * criteria = DetachedCriteria.forClass(Doctor.class); if (!"".equals(name))
 * criteria.add(Restrictions.eq("name", name)); if (!"".equals(city))
 * criteria.add(Restrictions.eq("address.city", city)); if
 * (!"".equals(speciality)) criteria.add(Restrictions.in("speciality",
 * speciality));
 * 
 * 
 * 
 * 
 * return (List<Doctor>) template.findByCriteria(criteria);
 * 
